...
尽管交叉设计消除了处理比较中实验单元之间的差异，但其他问题可能会以**延滞** (carryover) 或**残余** (residual) 效应的形式出现。例如，当首先给予处理 $A$ 并且在应用处理 $B$ 时其效应尚未消失，就会出现延滞效应。如果 $A$ 的延滞效应干扰了个体对处理 $B$ 的响应（或正向或负向），那么处理 $A$ 对处理 $B$ 有延滞效应。
