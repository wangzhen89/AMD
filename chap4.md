...
(split samples) 来生成两个观测结果，并将它们称为**重复** (replications)，而实际上它们是**子样本** (sub-samples) 或**重复测量** (repeated measures). **复制项必然不会提供与独立重复项相同的信息**。
...
其中 $\mathrm{AI}_i$ 表示的添加剂 I 第 $i$ 个水平的效应，$\mathrm{AII}_j$ 表示添加剂 II 第 $j$ 个水平的效应，$T_k$ 表示温度第 $k$ 个水平的效应，$\mathrm{Index}$ 表示由实验中观测到的 $25$ 种处理组合组成的指标集。
